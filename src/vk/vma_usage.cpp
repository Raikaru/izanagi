...
class VmaMutex {
   public:
    ...
    void      Lock() { gpu::mutex_lock(&m_mutex); }
    void      Unlock() { gpu::mutex_unlock(&m_mutex); }
    bool      TryLock() { return gpu::mutex_try_lock(&m_mutex); }

   private:
    gpu::mutex m_mutex = IZ_MUTEX_INIT;
};
