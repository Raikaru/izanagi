...
VkPipelineStageFlags2 bridge_pipeline_stage(StageFlags stage) {
    ...
    out |= any(stage & StageFlags::RasterColorOut) ? VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT : 0;
...
UsageFlags bridge_usage_flags(VkImageUsageFlags flags) {
    ...
    usage |= (flags & VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT) ? UsageFlags::ColorAttachment : UsageFlags::None;
    usage |= (flags & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT) ? UsageFlags::DepthStencilAttachment : UsageFlags::None;
    return usage;
}
...
VkImageUsageFlags bridge_usage_flags(UsageFlags usage) {
    ...
    flags |= any(usage & UsageFlags::DepthStencilAttachment) ? VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT : 0;
    return flags;
}
...
Span<const char> string_from_result(VkResult result) {
    switch (result) {
        ...
        default: return "VK_RESULT_UNKNOWN"_sv;
    }
}
